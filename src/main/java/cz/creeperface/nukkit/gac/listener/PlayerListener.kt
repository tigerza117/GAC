...
class PlayerListener(private val plugin: GTAnticheat) : Listener {
    ...
    fun onMove(e: PlayerMoveEvent) {
        ...
        if (cheatData.freeriding >= 1 || revert || p.onGround || p.riding != null || p.adventureSettings.get(AdventureSettings.Type.ALLOW_FLIGHT) || !NoCheatTask.isInAir(p, acData.antiCheatData)) {
            if (p.onGround) {
                cheatData.lastOnGround = time
